...
const doctorController = {
    ...
    getDoctorById: async function (req, res) { 
        try {
            ...
            opStatus('Doctor Fetched!', 200, 'Cannot Find Doctor!', result, res, true);
    ...
    createDoctor : async function (req, res) {
        try {
            ...
            if (!appData.full_name || !appData.email || !appData.date_of_birth || !appData.password || !appData.gender || !appData.phone_number || !appData.specialization || !appData.longitude || !appData.latitude) {
                return res.status(400).json({ error: 'Please fill all fields of the registration form!' });
            }
            ...
            const tableData = ['full_name', 'email', 'date_of_birth', 'password', 'gender', 'phone_number', 'specialization', 'longitude', 'latitude'];
            const result = await doctorModel.create(Object.values(appData), tableData);
...
const generateToken = (doctor) => { // doctor{ email, id}
