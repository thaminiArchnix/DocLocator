...
const Dashboard = () => {
    ...
    const fetchTodayAppointments = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3000/app/today/now/${userData.user.id}`
        );
        const todays = Object.values(response.data).filter(
  ...
  useEffect(() => {
    const checkAppointmentTime = async () => {
      ...
      const currentTimeString = `${currentTime.getHours()}:${currentTime
        .getMinutes()
        .toString()
        ...
        if (endTime === currentTimeString && app.status == "Pending") {
          ...
          try {
            ...
            const response = await axios.put(
              `http://localhost:3000/app/${app.appId}`,
              data
            );
            setMissed(true);
          } catch (error) {
    ...
    <>
      {loading ? (
        ...
        <div className="d-block">
          ...
          {startApp && (
            <div className="text-white bg-danger p-3">
              It is time for your appointment. Press Start to Begin!
            </div>
          )}
          {missed && (
            <div className="d-flex align-items-center justify-content-center flex-row text-white bg-danger p-3">
              You missed an appointment!{" "}
              <button
                className="m-3 btn btn-outline-light"
          ...
          <div className="row-sm-11 d-flex flex-wrap align-items-center justify-content-center">
            <div className="col-sm-6 text-center p-5">
              <h6>Welcome to DocLocator!</h6>{" "}
              <p>
                Whether you're in the clinic, at home, or on the go, use
                DocLocator to make informed decisions and deliver personalized
                care. Join our community of healthcare professionals dedicated
                to excellence and innovation. Together, let's shape the future
                of medicine and transform healthcare delivery for the better.
              </p>
            </div>
            <div className="col-sm-5 d-flex align-items-center justify-content-center">
              <img src={doctorImage} width="300" height="300" />
            </div>
          </div>
