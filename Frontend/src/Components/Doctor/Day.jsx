...
const Day = (props) => {
  ...
  const [filterDate, setFilter] = useState(props.filterdate);
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        ...
        const userapps = await Object.values(response.data).filter(
          (app) => app.docId == userData.user.id
        );
        const todays = await Object.values(userapps).filter(
          (app) => app.date == props.date
        );
        const nonCompleted = await Object.values(todays).filter(
          (app) => app.status !== "Completed"
        );
        const completedAppointments = await Object.values(todays).filter(
          (app) => app.status == "Completed"
        );
